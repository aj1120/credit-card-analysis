...
def add_risk_and_rewards(df: DataFrame) -> DataFrame:
    return df.withColumns({
        "updated_reward_points": col("reward_points") +
                                 spark_round(col("transaction_amount") / 10),
...
if __name__ == "__main__":
    spark = SparkSession.builder.appName("Credit Card Transactions Processor").getOrCreate()
    ...
    TEMP_GCS_BUCKET = GCS_BUCKET_NAME   # FIXED
    ...
    cardholders_df = spark.read.format("bigquery") \
        .option("table", BQ_CARDHOLDERS_TABLE).load()

    transactions_df = spark.read.json(json_file_path)  # FIXED

    # Add missing columns
    required_cols = ["risk_score", "merchant_name",
                     "merchant_location", "reward_points"]

    for c in required_cols:
        if c not in transactions_df.columns:
            transactions_df = transactions_df.withColumn(c, lit(None))
